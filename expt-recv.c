...
//	ioctl(socket_fd, SIOCSIFFLAGS, &ifr);
	...
	FILE *fpr;
	fpr = fopen("db20r-3.txt","a");
//	fprintf(fpr,"Current Timestamp(microsecs) SeqNum  Latitude Longitude \n");
...
*/
		...
		int64_t micros = time2.tv_sec * 1000000;
		// Adding full microseconds 
		micros += time2.tv_nsec/1000;
		// rounding up 
		if (time2.tv_nsec % 1000 >= 500) {
			++micros;
		}
		
		fprintf(fpr,"%u",micros);
		fprintf(fpr,"\t\t");
		//fprintf(fpr,"%u",times);
		//fprintf(fpr,"\t\t");
		fprintf(fpr,"%u",seq);

...
struct Blob1 {
        ...
        // pos      PositionLocal3D,
        uint32_t lat;    // lat         Latitude,              -- 4 bytes
        uint32_t longi;   // long        Longitude,             -- 4 bytes          // note: pick a different name to avoid collisions
...
printf("/n%d",ifr.ifr_name);	
	...
	BSM_head->msgID = DSRCmsgID_basicSafetyMessage;
        ...
        BSM_head->blob1.lat = 0x14DC9380;
        BSM_head->blob1.longi = 0x47868C00;
        BSM_head->blob1.elev = 0x2710;
        BSM_head->blob1.speed = 0x49C4;
...
*/
        int tot_len = strlen(data) + sizeof(struct BasicSafetyMessage); 

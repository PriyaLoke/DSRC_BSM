...
void getFixedData(struct BasicSafetyMessage **BSM_head){
	...
	(*BSM_head)->blob1.accuracy = 0x00000000;
	(*BSM_head)->blob1.angle = 0x0;
}

void getGpsSensorData(struct BasicSafetyMessage **BSM_head, struct gps_data_t *gpsdata, uint8_t *msgNum){
	(*BSM_head)->blob1.lat =((*gpsdata).fix.latitude);
	(*BSM_head)->blob1.longi =((*gpsdata).fix.longitude);
	//BSM_head->blob1.lat=40.499;
